...
function initParticles() {
    ...
    class Particle {
        ...
        update() {
            ...
            this.y += Math.cos(this.wobble) * 0.1;

            // Nếu ra khỏi màn hình thì reset
            ...
            // Tương tác với chuột - hạt bay ra xa khi chuột đến gần
        ...
        draw() {
            ...
            ctx.fillStyle = this.color + Math.floor(this.opacity * 200).toString(16).padStart(2, '0');
            ctx.fill();
        }
    }
    ...
    // Vẽ các chấm sáng lớn (ngôi sao)
    function drawStars() {
        const starCount = 20;
        for (let i = 0; i < starCount; i++) {
            const x = (i * 137.5 + 50) % canvas.width;
            const y = (i * 97.3 + 30) % canvas.height;
            const size = Math.sin(i * 1.7) * 1 + 0.5;
            const opacity = Math.sin(i * 2.3 + Date.now() * 0.0003) * 0.15 + 0.2;
